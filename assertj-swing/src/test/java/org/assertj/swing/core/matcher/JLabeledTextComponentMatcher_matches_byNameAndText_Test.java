 ...
 * @author Yvonne Wang
 */
public class JLabeledTextComponentMatcher_matches_byNameAndText_Test extends EDTSafeTestCase {
  @Test
  public void should_Return_True_If_Name_And_Text_Are_Equal_To_Expected() {
    JLabeledTextComponentMatcher matcher = JLabeledTextComponentMatcher.withName("textField").andText("Hello");
  ...
  public void should_Return_True_If_Name_Is_Equal_To_Expected_And_Text_Matches_Pattern() {
    JLabeledTextComponentMatcher matcher = JLabeledTextComponentMatcher.withName("textField").andText("He.*");
    JTextField textField = textField().withName("textField").withText("Hello").createNew();
    assertThat(matcher.matches(textField)).isTrue();
  }
}
