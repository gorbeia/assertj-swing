 ...
 * @author Yvonne Wang
 */
public class JLabeledTextComponentMatcher_matches_byLabelTextPattern_Test extends EDTSafeTestCase {

    @Test
    public void should_Return_True_If_Text_Matches_Pattern() {
        JLabeledTextComponentMatcher matcher = JLabeledTextComponentMatcher.withLabelText(Pattern.compile("He.*"));
    ...
    public void should_Return_False_If_Text_Does_Not_Match_Pattern() {
        JLabeledTextComponentMatcher matcher = JLabeledTextComponentMatcher.withLabelText(Pattern.compile("Hello"));
        LabeledTextComponentWindow window = labeledTextField().withLabelText("Bye").createNew(getClass());
        assertThat(matcher.matches(window.getTextField())).isFalse();
    }
}
