 ...
 * @author Yvonne Wang
 */
public class JLabeledTextComponentMatcher_matches_byTextPattern_Test extends EDTSafeTestCase {
  @Test
  public void should_Return_True_If_Text_Matches_Pattern() {
    JLabeledTextComponentMatcher matcher = JLabeledTextComponentMatcher.withText(Pattern.compile("He.*"));
  ...
  public void should_Return_False_If_Text_Does_Not_Match_Pattern() {
    JLabeledTextComponentMatcher matcher = JLabeledTextComponentMatcher.withText(Pattern.compile("Hello"));
    JTextField textField = textField().withText("Bye").createNew();
    assertThat(matcher.matches(textField)).isFalse();
  }
}
