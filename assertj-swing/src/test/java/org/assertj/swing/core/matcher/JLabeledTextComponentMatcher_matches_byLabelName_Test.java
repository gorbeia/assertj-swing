 ...
 * @author Yvonne Wang
 */
public class JLabeledTextComponentMatcher_matches_byLabelName_Test extends EDTSafeTestCase {

    @Test
    public void should_Return_True_If_Name_Is_Equal_To_Expected() {
        String name = "textField";
        LabeledTextComponentWindow window = labeledTextField().withLabelName(name).createNew(getClass());
        JLabeledTextComponentMatcher matcher = JLabeledTextComponentMatcher.withLabelName(name);
        assertThat(matcher.matches(window.getTextField())).isTrue();
    }

    @Test
    public void should_Return_False_If_Name_Is_Not_Equal_To_Expected() {
        LabeledTextComponentWindow window = labeledTextField().withLabelName("label").createNew(getClass());
        JLabeledTextComponentMatcher matcher = JLabeledTextComponentMatcher.withLabelName("textField");
